...
class Model():
    def __init__(self):
        ...
        # 2d
        self.normal_vector = self.display_plate_basis[0] * \
            self.display_plate_basis[1]
    ...
    def get_display_vector_on_plate_of_display(self, vector):
        temp = -(self.normal_vector.x*vector.x +\
                 self.normal_vector.y*vector.y +
                 self.normal_vector.z*vector.z) / \
        ...
        matrix = Matrix(3, 3, self.display_plate_basis[0].x,
                        self.display_plate_basis[1].x,
                        displayed_vector.x,
                        self.display_plate_basis[0].y,
                        self.display_plate_basis[1].y,
                        displayed_vector.y,
                        self.display_plate_basis[0].z,
                        self.display_plate_basis[1].z,
                        displayed_vector.z)
    ...
    def update_matrix_of_display(self, ort_matrix: Matrix):
        if not ort_matrix:
            ...
            self.matrix_of_display = Matrix(
                2, 3, a[0], b[0], c[0], a[1], b[1], c[1])
        else:
            #self.matrix_of_display = ort_matrix.transpose()*self.matrix_of_display*ort_matrix
            self.display_plate_basis[0] = Vector3(
                *((ort_matrix*self.display_plate_basis[0].to_matrix()).to_tuple()))
            self.display_plate_basis[1] = Vector3(
                *((ort_matrix*self.display_plate_basis[1].to_matrix()).to_tuple()))
            self.normal_vector = Vector3(
                *((ort_matrix*self.normal_vector.to_matrix()).to_tuple()))
            self.update_matrix_of_display(None)
