...
class RedactorWindow(QtWidgets.QMainWindow):
    ...
    def paint_point(self, point, painter):
        display_coord = (self.model.matrix_of_display * Matrix(3, 1, point.x, point.y, point.z)).to_tuple()
        ...
        painter.drawEllipse(self.points_display_table[point][0] - width / 2, self.points_display_table[point][1] - width / 2, width, width)

    def paint_line(self, line, painter):
        painter.pen().setWidth(max(5, 2*line.radius))
        painter.drawLine(*(self.points_display_table[line.start]), *(self.points_display_table[line.end]))
    ...
    def update_object_to_interact(self, event):
        ...
        for obj in self.points_display_table:
            distance = math.sqrt((event.x() - self.points_display_table[obj][0])*(event.x() - self.points_display_table[obj][0]) +
                                (event.y() - self.points_display_table[obj][1])*(event.y() - self.points_display_table[obj][1]))   #it doesn't works
